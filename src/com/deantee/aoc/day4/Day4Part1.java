...
public class Day4Part1 {
  ...
  public static void main(String[] args) {
    try {
      ...
      int number = 0;
      while (bingoCards.stream().noneMatch(BingoCard::isBingo)) {
        number = numbers.remove(0);
        for (BingoCard card : bingoCards) {
          card.mark(number);
        }
      }
      System.out.println(bingoCards.stream().filter(BingoCard::isBingo).toList().get(0).getScore(number));
    } catch (IOException e) {
      e.printStackTrace();
    }
  }
